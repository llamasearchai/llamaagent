...
from llamaagent.benchmarks.baseline_agents import (
    BaselineAgentFactory, BaselineType, VanillaReactAgent, 
    PreActOnlyAgent, SEMOnlyAgent
)
...
class TestBaselineAgentFactory:
    ...
    def test_create_vanilla_react_agent(self):
        ...
        with patch('llamaagent.benchmarks.baseline_agents.ToolRegistry') as mock_registry:
            with patch('llamaagent.benchmarks.baseline_agents.get_all_tools', return_value=[]):
                ...
                assert agent.config.spree_enabled == False
    ...
    def test_create_preact_only_agent(self):
        ...
        with patch('llamaagent.benchmarks.baseline_agents.ToolRegistry') as mock_registry:
            with patch('llamaagent.benchmarks.baseline_agents.get_all_tools', return_value=[]):
                ...
                assert agent.config.spree_enabled == True
    ...
    async def test_execute_overrides_spree_setting(self):
        ...
        with patch('llamaagent.benchmarks.baseline_agents.ToolRegistry'):
            with patch('llamaagent.benchmarks.baseline_agents.get_all_tools', return_value=[]):
                ...
                with patch.object(VanillaReactAgent.__bases__[0], 'execute') as mock_super_execute:
                    with patch.object(agent, 'add_trace') as mock_add_trace:
                        from llamaagent.agents.base import AgentResponse
                        mock_response = AgentResponse(content="test", success=True, tokens_used=0, trace=[])
                        ...
                        mock_add_trace.assert_called_once_with("baseline_type", {"type": BaselineType.VANILLA_REACT})
    ...
    async def test_assess_resource_need_always_true(self):
        ...
        with patch('llamaagent.benchmarks.baseline_agents.ToolRegistry'):
            with patch('llamaagent.benchmarks.baseline_agents.get_all_tools', return_value=[]):
                ...
                step = PlanStep(step_id=1, description="test", required_information="info", expected_outcome="result")
                
                with patch.object(agent, 'add_trace') as mock_add_trace:
                    result = await agent._assess_resource_need(step)
                    
                    assert result == True
                    mock_add_trace.assert_called_once_with("resource_assessment_override", {
    ...
    async def test_execute_spre_pipeline_override(self):
        ...
        with patch('llamaagent.benchmarks.baseline_agents.ToolRegistry'):
            with patch('llamaagent.benchmarks.baseline_agents.get_all_tools', return_value=[]):
                ...
                with patch.object(agent, 'add_trace') as mock_add_trace:
                    with patch.object(agent, '_execute_plan_with_resource_assessment') as mock_execute_plan:
                        mock_execute_plan.return_value = [{"result": "Task completed"}]
                        
                        result = await agent._execute_spre_pipeline("test task")
                        
                        assert result == "Task completed"
                        mock_add_trace.assert_called_once_with("sem_only_execution", {"baseline_type": BaselineType.SEM_ONLY})
