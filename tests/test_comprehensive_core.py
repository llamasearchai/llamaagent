...
from src.llamaagent.agents.base import AgentConfig, BaseAgent
# Integration imports
from src.llamaagent.api import create_app
from src.llamaagent.benchmarks import GAIABenchmark, SPREEvaluator
from src.llamaagent.cache import CacheManager, ResultCache
from src.llamaagent.cli.interactive import InteractiveCLI
from src.llamaagent.data_generation import GDTGenerator, SPREGenerator
...
from src.llamaagent.types import (AgentResponse, AgentTrace, ConversationTurn,
                                  ToolCall, ToolResult)
...
class TestCoreAgents:
    ...
    async def test_react_agent_execution(self, mock_provider, calculator_tool):
        ...
        agent = ReactAgent(
            name="TestReActAgent", provider=mock_provider, tools=[calculator_tool]
        )
    ...
    async def test_agent_with_memory(self, mock_provider):
        ...
        agent = ReactAgent(name="MemoryAgent", provider=mock_provider)
    ...
    async def test_agent_tool_execution(
        self, mock_provider, calculator_tool, python_tool
    ):
    ...
    async def test_agent_error_handling(self, mock_provider):
        ...
        agent = ReactAgent(name="ErrorAgent", provider=mock_provider)
...
class TestTools:
    ...
    def test_custom_tool_creation(self):
        ...
        @Tool(name="custom_tool", description="A custom tool for testing")
...
class TestMemory:
    ...
    def test_memory_entry_creation(self):
        ...
        entry = MemoryEntry(
            role="user", content="Test message", metadata={"timestamp": "2024-01-01"}
        )
        ...
        with patch('src.llamaagent.api.ReactAgent') as mock_agent_class:
            ...
            mock_agent.run = AsyncMock(
                return_value=AgentResponse(
                    success=True, response="Test response", trace=AgentTrace(steps=[])
                )
            )
            mock_agent_class.return_value = mock_agent

            response = await client.post(
                "/chat", json={"message": "Hello", "agent_name": "test_agent"}
            )
...
class TestCLI:
    ...
    def mock_agent(self):
        ...
        agent.run = AsyncMock(
            return_value=AgentResponse(
                success=True, response="CLI test response", trace=AgentTrace(steps=[])
            )
        )
        return agent
...
class TestBenchmarks:
    ...
    async def test_spre_evaluator(self):
        ...
        mock_agent.run = AsyncMock(
            return_value=AgentResponse(
                success=True, response="42", trace=AgentTrace(steps=[])
            )
        )
    ...
    async def test_gaia_benchmark(self):
        ...
        mock_agent.run = AsyncMock(
            return_value=AgentResponse(
                success=True, response="Test answer", trace=AgentTrace(steps=[])
            )
        )
...
class TestIntegration:
    ...
    async def test_benchmark_workflow(self):
        ...
        # Create agent
        agent = ReactAgent(name="BenchmarkAgent", provider=MockProvider())
...
class TestPerformance:
    ...
    async def test_concurrent_agent_execution(self):
        ...
        agents = [
            ReactAgent(name=f"Agent{i}", provider=MockProvider()) for i in range(10)
        ]

        # Run agents concurrently
        tasks = [agent.run(f"Query {i}") for i, agent in enumerate(agents)]
    ...
    async def test_cache_performance(self):
        ...
        # Set many values
        set_tasks = [cache.set(f"key_{i}", f"value_{i}", ttl=60) for i in range(100)]
        await asyncio.gather(*set_tasks)

        # Get many values
        get_tasks = [cache.get(f"key_{i}") for i in range(100)]
