...
from llamaagent.benchmarks.spre_evaluator import (
    TaskResult, BenchmarkResult, SPREEvaluator
)
...
class TestBenchmarkResult:
    ...
    def test_benchmark_result_creation(self):
        ...
        task_results = [
            TaskResult(
                task_id="task_1", question="Q1", expected_answer="A1", actual_answer="A1",
                success=True, execution_time=1.0, tokens_used=10, api_calls=1,
                reasoning_tokens=5, baseline_type="test"
            ),
            TaskResult(
                task_id="task_2", question="Q2", expected_answer="A2", actual_answer="Wrong",
                success=False, execution_time=2.0, tokens_used=20, api_calls=2,
                reasoning_tokens=10, baseline_type="test"
            )
        ]
    ...
    def test_success_rate_calculation(self):
        ...
        task_results = [
            ...
            TaskResult(
                task_id="task_3", question="Q3", expected_answer="A3", actual_answer="Wrong",
                success=False, execution_time=1.5, tokens_used=15, api_calls=1,
                reasoning_tokens=7, baseline_type="test"
            )
        ]
    ...
    def test_avg_api_calls(self):
        ...
        task_results = [
            ...
            TaskResult(
                task_id="task_2", question="Q2", expected_answer="A2", actual_answer="A2",
                success=True, execution_time=2.0, tokens_used=20, api_calls=3,
                reasoning_tokens=10, baseline_type="test"
            )
        ]
    ...
    def test_avg_latency(self):
        ...
        task_results = [
            ...
            TaskResult(
                task_id="task_2", question="Q2", expected_answer="A2", actual_answer="A2",
                success=True, execution_time=3.0, tokens_used=20, api_calls=2,
                reasoning_tokens=10, baseline_type="test"
            )
        ]
    ...
    def test_avg_tokens(self):
        ...
        task_results = [
            ...
            TaskResult(
                task_id="task_2", question="Q2", expected_answer="A2", actual_answer="A2",
                success=True, execution_time=2.0, tokens_used=30, api_calls=2,
                reasoning_tokens=10, baseline_type="test"
            )
        ]
    ...
    def test_efficiency_ratio(self):
        ...
        task_results = [
            TaskResult(
                task_id="task_1", question="Q1", expected_answer="A1", actual_answer="A1",
                success=True, execution_time=1.0, tokens_used=10, api_calls=2,
                reasoning_tokens=5, baseline_type="test"
            ),
            TaskResult(
                task_id="task_2", question="Q2", expected_answer="A2", actual_answer="Wrong",
                success=False, execution_time=2.0, tokens_used=20, api_calls=4,
                reasoning_tokens=10, baseline_type="test"
            )
        ]
    ...
    def test_efficiency_ratio_zero_api_calls(self):
        ...
        task_results = [
            TaskResult(
                task_id="task_1", question="Q1", expected_answer="A1", actual_answer="A1",
                success=True, execution_time=1.0, tokens_used=10, api_calls=0,
                reasoning_tokens=5, baseline_type="test"
            )
        ]
        
        result = BenchmarkResult("test", "TestAgent", task_results)
        assert result.efficiency_ratio == 100.0  # Should return success_rate when avg_api_calls is 0
    ...
    def test_get_stats_by_difficulty(self):
        ...
        task_results = [
            ...
            TaskResult(
                task_id="task_3", question="Q3", expected_answer="A3", actual_answer="A3",
                success=True, execution_time=3.0, tokens_used=30, api_calls=3,
                reasoning_tokens=15, baseline_type="test"
            )
        ]
        ...
        with patch('pathlib.Path.exists', return_value=True):
            with patch('builtins.open', mock_open(read_data=json.dumps(mock_benchmark_data))):
    ...
    def test_get_stats_by_difficulty_empty_category(self):
        ...
        with patch('llamaagent.benchmarks.spre_evaluator.GAIABenchmark') as mock_benchmark_class:
            mock_benchmark = Mock()
...
class TestSPREEvaluator:
    ...
    async def test_run_full_evaluation_no_tasks(self):
        ...
        with patch.object(evaluator.benchmark, 'get_tasks', return_value=[]):
            with pytest.raises(ValueError, match="No tasks found matching filter criteria"):
    ...
    def test_evaluate_answer_exact_match(self):
        ...
        assert evaluator._evaluate_answer("42", "42") == True
        assert evaluator._evaluate_answer("hello world", "hello world") == True
    
    def test_evaluate_answer_case_insensitive(self):
        """Test _evaluate_answer with case differences."""
        evaluator = SPREEvaluator()
        
        assert evaluator._evaluate_answer("Hello", "hello") == True
        assert evaluator._evaluate_answer("WORLD", "world") == True
    ...
    def test_evaluate_answer_numeric_extraction(self):
        ...
        assert evaluator._evaluate_answer("3.14", "Pi is approximately 3.14159") == True
        assert evaluator._evaluate_answer("100", "The result equals 100%") == True
    ...
    def test_evaluate_answer_no_match(self):
        ...
        assert evaluator._evaluate_answer("42", "100") == False
        assert evaluator._evaluate_answer("hello", "goodbye") == False
    ...
    async def test_save_results(self):
        ...
        results = {
            "test_baseline": BenchmarkResult("test_baseline", "TestAgent", [])
        }
    ...
    def test_generate_comparison_report(self):
        ...
        with patch.object(BenchmarkResult, 'get_stats_by_difficulty', return_value={"easy": {"success_rate": 100.0, "count": 1}}):
            with patch('builtins.open', create=True) as mock_open:
    ...
    async def test_run_single_baseline_evaluation(self):
        ...
        mock_tasks = [
            GAIATask("task_1", "What is 2+2?", "4", "easy", 1, "math")
        ]
        
        with patch.object(evaluator.benchmark, 'get_tasks', return_value=mock_tasks):
            with patch('llamaagent.benchmarks.spre_evaluator.BaselineAgentFactory') as mock_factory:
                with patch.object(evaluator, '_evaluate_agent_on_tasks') as mock_evaluate:
                    mock_agent = Mock()
    ...
    async def test_evaluate_agent_on_tasks_success(self):
        ...
        with patch.object(evaluator, '_evaluate_answer', return_value=True):
            result = await evaluator._evaluate_agent_on_tasks(mock_agent, tasks, "test_baseline")
            
            assert result.baseline_type == "test_baseline"
            assert result.agent_name == "TestAgent"
            assert len(result.task_results) == 1
            assert result.task_results[0].success == True
    ...
    async def test_evaluate_agent_on_tasks_exception(self):
        ...
        assert len(result.task_results) == 1
        assert result.task_results[0].success == False
            ...
            with patch('llamaagent.benchmarks.spre_evaluator.BaselineAgentFactory') as mock_factory:
                with patch.object(evaluator, '_evaluate_agent_on_tasks') as mock_evaluate:
                    with patch.object(evaluator, '_save_results') as mock_save:
                        with patch.object(evaluator, '_generate_comparison_report') as mock_report:
                            with patch('builtins.print'):  # Mock print statements
                                mock_agent = Mock()
                                mock_factory.create_agent.return_value = mock_agent
                                mock_factory.get_all_baseline_types.return_value = ["vanilla_react", "spre_full"]
                                
                                # Create a result with task data that yields the expected metrics
                                task_results = [
                                    TaskResult(
                                        task_id="task_1", question="Q1", expected_answer="A1", actual_answer="A1",
                                        success=True, execution_time=1.2, tokens_used=10, api_calls=2,
                                        reasoning_tokens=5, baseline_type="test_baseline"
                                    )
                                ]
                                expected_result = BenchmarkResult("test_baseline", "TestAgent", task_results)
                                mock_evaluate.return_value = expected_result
    ...
    def test_evaluate_answer_value_error_handling(self):
        ...
        with patch('re.findall') as mock_findall:
            ...
            assert result == False  # Should fall back to False when ValueError occurs 