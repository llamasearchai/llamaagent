...
class ParenthesesFixer:
    ...
    def fix_common_patterns(self, content: str) -> str:
        ...
        fixes = [
            ...
            (r'ReactAgent\(config=AgentConfig\([^)]*\)(?!\))', lambda m: m.group(0) + ')'),
            ...
            (r'asyncio\.run\([^)]*\)(?!\))', lambda m: m.group(0) + ')' if not m.group(0).endswith('))') else m.group(0)),
            
            # Fix specific patterns found in the codebase
            (r'raise HTTPException\([^)]*detail=str\(e\)(?!\))', lambda m: m.group(0) + ')'),
            (r'datetime\.now\(timezone\.utc\)\)(?!\))', lambda m: m.group(0)[:-1]),  # Remove extra parenthesis
            (r'return\s+StructuredLogger\(name\)\)', lambda m: m.group(0)[:-1]),  # Remove extra parenthesis
        ]
        ...
        try:
            ...
            if fixed_content != original_content:
                ...
                try:
                    ...
                    logger.warning(f"Fixed content still has syntax errors: {file_path} - {e}")
                    return False
            else:
