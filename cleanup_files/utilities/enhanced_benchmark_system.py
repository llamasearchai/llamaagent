...
class AdvancedScorer:
    ...
    def numerical_accuracy_score(
        self, actual: str, expected: str, tolerance: float = 0.01
    ) -> float:
    ...
    def multi_criteria_score(
        self, actual: str, expected: str, criteria: Dict[str, float]
    ) -> float:
    ...
    def score_task(self, task: BenchmarkTask, actual_answer: str) -> float:
        ...
        if task.evaluation_metric == EvaluationMetric.EXACT_MATCH:
            ...
            task.evaluation_metric == EvaluationMetric.CUSTOM_SCORER
            and task.custom_scorer
        ):
...
class BenchmarkDataGenerator:
    ...
    def generate_suite(
        self, name: str, task_counts: Dict[TaskType, int]
    ) -> BenchmarkSuite:
...
class BenchmarkRunner:
    ...
    async def run_benchmark(
        self, suite: BenchmarkSuite, agent: ReactAgent, parallel: bool = True
    ) -> BenchmarkResult:
        ...
        result = BenchmarkResult(
            ...
            metadata={
                ...
                "average_execution_time": statistics.mean(
                    [r.execution_time for r in task_results]
                )
                if task_results
                else 0.0,
        ...
        logger.info(
            f"Benchmark completed: {overall_score:.2%} success rate in {execution_time:.2f}s"
        )

        return result

    async def _run_sequential(
        self, tasks: List[BenchmarkTask], agent: ReactAgent
    ) -> List[TaskResult]:
    ...
    async def _run_parallel(
        self, tasks: List[BenchmarkTask], agent: ReactAgent, max_workers: int = 5
    ) -> List[TaskResult]:
...
class BenchmarkAnalyzer:
    ...
    def _analyze_errors(self, result: BenchmarkResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": len(failed_tasks) / len(result.task_results)
            if result.task_results
            else 0.0,
    ...
    def _analyze_efficiency(self, result: BenchmarkResult) -> Dict[str, Any]:
        ...
        return {
            "tokens_per_task": total_tokens / len(result.task_results)
            if result.task_results
            else 0.0,
            "api_calls_per_task": total_api_calls / len(result.task_results)
            if result.task_results
            else 0.0,
            "tokens_per_successful_task": total_tokens / successful_tasks
...
class BenchmarkReporter:
    ...
    def generate_report(
        self, result: BenchmarkResult, output_path: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        report = {
            ...
            "executive_summary": {
                ...
                "average_execution_time": analysis["time_analysis"].get(
                    "average_time", 0.0
                ),
    ...
    def _generate_key_insights(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if task_type_perf:
            best_type = max(
                task_type_perf.keys(), key=lambda k: task_type_perf[k]["success_rate"]
            )
            worst_type = min(
                task_type_perf.keys(), key=lambda k: task_type_perf[k]["success_rate"]
            )
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if overall["success_rate"] < 0.7:
            recommendations.append(
                "Consider improving agent's problem-solving capabilities"
            )
        ...
        if efficiency.get("tokens_per_task", 0) > 1000:
            recommendations.append(
                "Consider reducing token usage for better efficiency"
            )

        return recommendations

    def generate_comparison_report(
        self, results: List[BenchmarkResult], output_path: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        report = {
            ...
            "winner": comparison["performance_ranking"][0]
...
class EnhancedBenchmarkSystem:
    ...
    def _create_built_in_suites(self) -> Dict[str, BenchmarkSuite]:
        ...
        suites["mathematics"] = self.data_generator.generate_suite(
            "Mathematics Evaluation", {TaskType.MATHEMATICAL: 10}
        )

        # Programming-focused suite
        suites["programming"] = self.data_generator.generate_suite(
            "Programming Evaluation", {TaskType.PROGRAMMING: 8}
        )

        return suites

    async def run_full_evaluation(
        self, agent: ReactAgent, suite_name: str = "comprehensive"
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                "overall_score": result.overall_score,
                "success_rate": len([r for r in result.task_results if r.success])
    ...
    async def compare_agents(
        self, agents: List[ReactAgent], suite_name: str = "comprehensive"
    ) -> Dict[str, Any]:
