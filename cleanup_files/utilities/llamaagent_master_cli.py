...
from src.llamaagent.planning import (ExecutionEngine, Task, TaskPlan,
                                     TaskPlanner, TaskPriority, TaskStatus)
...
class MasterCLI:
        ...
        for name, role, description in agent_configs:
            ...
            try:
                ...
                provider = create_provider(
                    provider_type=self.current_provider, **provider_config
                )
                ...
                logger.info(
                    f"Initialized agent: {name} with provider: {self.current_provider}"
                )
    ...
    def show_banner(self):
        ...
        self.console.print(
            Panel(
                ...
                + "\n"
                + Text(
                    "Master CLI with Dynamic Task Planning & Scheduling",
                    style="bold white",
                )
                + "\n"
    ...
    def show_main_menu(self):
        ...
        table = Table(
            title="Master CLI Features", show_header=True, header_style="bold magenta"
        )
    ...
    async def _create_task_plan(self):
        ...
        if not tasks:
            self.console.print(
                "[yellow]No tasks added. Plan creation cancelled.[/yellow]"
            )
            return
    ...
    async def _edit_task_plan(self):
        ...
        for task in selected_plan.tasks.values():
            table.add_row(task.id[:8], task.name, task.priority.name, task.status.name)
    ...
    async def _agent_chat_interface(self):
        ...
        while True:
            try:
                ...
                self.console.print(
                    f"[bold green]{agent_name}[/bold green]: {response.content}"
                )
    ...
    async def _dashboard_interface(self):
        ...
        status_table.add_row(
            "Tools", "Ready", f"{len(self.tools.list_names())} available"
        )
    ...
    async def _configure_model_settings(self):
        ...
        settings_table = Table(
            title=f"Current Settings ({self.current_provider})", show_header=True
        )
            ...
            for key, current_value in config.items():
                if key == "api_key" and current_value == "":
                    new_value = Prompt.ask(
                        f"Enter {key} (hidden)", password=True, default=""
                    )
                else:
                ...
                if key in [
                    ...
                    try:
                        ...
                        self.console.print(
                            f"[red]Invalid integer value for {key}[/red]"
                        )
                        continue
    ...
    async def _show_system_info(self):
        ...
        info_table.add_row(
            "Model",
            self.provider_configs[self.current_provider].get("model_name", "Unknown"),
        )
        ...
        info_table.add_row(
            "Provider Override", os.getenv("LLAMAAGENT_LLM_PROVIDER", "None")
        )
    ...
    async def _reinitialize_agents(self):
        ...
        self.console.print(
            "[yellow]Reinitializing agents with new configuration...[/yellow]"
        )
    ...
    async def _test_agents(self):
        ...
        for name, agent in self.agents.items():
            try:
                ...
                self.console.print(
                    f"[green]{name}: {response.content[:100]}...[/green]"
                )
            except Exception as e:
    ...
    async def _system_diagnostics(self):
        ...
        diagnostics = [
            ...
            (
                "Tools",
                len(self.tools.list_names()) > 0,
                f"{len(self.tools.list_names())} available",
            ),
