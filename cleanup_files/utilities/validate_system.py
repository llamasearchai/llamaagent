...
class SystemValidator:
    ...
    async def validate_imports(self):
        ...
        # Core imports
        try:
            from llamaagent_master_program import (CreateMasterTaskRequest,
                                                   MasterOrchestrator)
        ...
        # Planning imports
        try:
            from llamaagent.planning.task_planner import Task, TaskPlanner
    ...
    async def validate_functionality(self):
        ...
        try:
            ...
            self.check(
                "Task creation", result.get("success", False), result.get("error", "")
            )
        ...
        try:
            ...
            config = SpawnConfig(
                agent_config=AgentConfig(
                    name="ValidationAgent", role=AgentRole.GENERALIST
                )
            )

            result = await spawner.spawn_agent(task="Validation test", config=config)
    ...
    async def validate_task_planning(self):
        ...
        try:
            ...
            plan = planner.create_plan(
                goal="Validate task planning system", auto_decompose=True
            )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    ...
                    self.check(
                        "API server",
                        False,
                        "Not running (start with: python3 llamaagent_master_program.py server)",
                        warning=True,
                    )
