...
try:
    ...
    from llamaagent.integration.openai_agents import (
        OpenAIAgentMode, OpenAIIntegrationConfig, create_openai_integration)
...
class MasterProgramManager:
    ...
    async def create_agent(self, config: AgentCreateRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create agent config
            agent_config = AgentConfig(name=config.name, temperature=0.1)
    ...
    async def execute_task(self, request: TaskRequest) -> TaskResponse:
        ...
        if request.agent_name not in self.agents:
            return TaskResponse(
                task_id="error", status="failed", error="Agent not found"
            )
        ...
        try:
            ...
            task_input = TaskInput(id=f"task_{int(time.time())}", task=request.task)
            ...
            return TaskResponse(
                ...
                metadata=getattr(result.result, 'metadata', {})
                if result.result
                else None,
            )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tools_available": self.tools_registry.list_tools()
            if hasattr(self.tools_registry, 'list_tools')
            else [],
        }
        ...
        agents_info.append(
            {
                ...
                "tools": manager.tools_registry.list_tools()
                if hasattr(manager.tools_registry, 'list_tools')
                else [],
...
def server(host, port, reload):
    ...
    console.print(
        f"[bold green]Starting LlamaAgent Master Program Server on {host}:{port}[/bold green]"
    )
    uvicorn.run(
        "master_program:app", host=host, port=port, reload=reload, log_level="info"
    )
        ...
        with Progress(
            ...
            for i, task in enumerate(demo_tasks):
                task_progress = progress.add_task(
                    f"Executing task {i+1}: {task[:50]}...", total=1
                )

                request = TaskRequest(agent_name="demo_agent", task=task, mode="hybrid")
                ...
                if response.result:
                    console.print(
                        f"[bold]Result:[/bold] {response.result.get('data', 'No data')}"
                    )
                if response.error:
...
def test():
    ...
    if result.returncode == 0:
        ...
        console.print(
            f"[bold red]Tests failed with exit code {result.returncode}[/bold red]"
        )
...
def status():
    ...
    async def show_status():
        ...
        status_table.add_row(
            "OpenAI Agents SDK",
            "Available" if status["openai_agents_available"] else "Not Available",
        )
        status_table.add_row(
            "Imports", "Available" if status["imports_available"] else "Not Available"
        )
