...
class MasterSystemOrchestrator:
        ...
        try:
            ...
            if CORE_MODULES_AVAILABLE:
                try:
                    ...
                    self.logger.warning(
                        f" Metrics collector initialization failed: {e}"
                    )
            ...
            for test_name, test_func in test_suite:
                try:
                    ...
                    if result["passed"]:
                        ...
                        self.logger.warning(
                            f" {test_name} test failed: {result.get('error', 'Unknown error')}"
                        )
    ...
    async def generate_system_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "performance_metrics": {
                ...
                "test_completion_time": self.test_results.get("summary", {}).get(
                    "duration", 0
                ),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not CORE_MODULES_AVAILABLE:
            recommendations.append(
                "Install missing core modules for full functionality"
            )

        if not ADVANCED_MODULES_AVAILABLE:
            recommendations.append(
                "Install missing advanced modules for enhanced features"
            )
        ...
        if self.test_results:
            ...
            if success_rate < 100:
                recommendations.append(
                    f"Address failed tests (current success rate: {success_rate:.1f}%)"
                )
    ...
    async def run_interactive_cli(self):
        ...
        self.console.print(
            Panel.fit(
                "LlamaAgent LlamaAgent Master System - Interactive CLI",
                style="bold blue",
            )
        )
        ...
        while True:
            try:
                command = (
                    self.console.input("\n[bold cyan]llamaagent>[/bold cyan] ")
                    .strip()
                    .lower()
                )
    ...
    async def _show_status(self):
        ...
        status_table.add_row(
            ...
            f"Uptime: {(datetime.now(timezone.utc) - self.start_time).total_seconds():.1f}s"
            if self.start_time
            else "Not started",
        )
        ...
        components = [
            (
                "Core Modules",
                " Available" if CORE_MODULES_AVAILABLE else " Missing",
                "LLM, Agents, Tools",
            ),
            (
                "Advanced Modules",
                " Available" if ADVANCED_MODULES_AVAILABLE else " Missing",
                "Cache, Router, Security",
            ),
            (
                "Metrics",
                " Active" if self.metrics_collector else " Inactive",
                "Performance monitoring",
            ),
            (
                "Health Check",
                " Active" if self.health_checker else " Inactive",
    ...
    async def _generate_report_interactive(self):
        ...
        self.console.print(
            f"Test Success Rate: [bold]{report['test_results'].get('summary', {}).get('success_rate', 0):.1f}%[/bold]"
        )
...
async def main():
    ...
    try:
        ...
        console.print(
            f"[green] Tests completed: {test_results['passed']}/{test_results['total_tests']} passed ({success_rate:.1f}%)[/green]"
        )
        ...
        console.print(
            f"• Overall Health: [bold]{report['system_status']['health_status']}[/bold]"
        )
        console.print(
            f"• Core Modules: [bold]{'' if CORE_MODULES_AVAILABLE else ''}[/bold]"
        )
        console.print(
            f"• Advanced Modules: [bold]{'' if ADVANCED_MODULES_AVAILABLE else ''}[/bold]"
        )
