...
try:
    ...
    from llamaagent.integration.openai_agents_complete import (
        OpenAIAgentsManager, create_openai_agent_manager)
    from llamaagent.llm.factory import create_provider
    from llamaagent.planning.task_planner import (Task, TaskDecomposer,
                                                  TaskPlan, TaskPlanner,
                                                  TaskPriority, TaskStatus)
    from llamaagent.spawning.agent_spawner import (AgentHierarchy,
                                                   AgentRelationship,
                                                   AgentSpawner, SpawnConfig)
    from llamaagent.tools import ToolRegistry, get_all_tools
    from llamaagent.types import TaskInput, TaskOutput, TaskResult
    from llamaagent.types import TaskStatus as TypeTaskStatus
...
class CreateMasterTaskRequest(BaseModel):
    ...
    auto_decompose: bool = Field(
        True, description="Automatically decompose into subtasks"
    )
    auto_spawn: bool = Field(
        True, description="Automatically spawn agents for subtasks"
    )
...
class MasterOrchestrator:
    ...
    def _initialize_tools(self) -> ToolRegistry:
        ...
        logger.info(
            f"Initialized {len(registry.tools) if hasattr(registry, 'tools') else 0} tools"
        )
        return registry
    ...
    async def create_master_task(
        self, request: CreateMasterTaskRequest
    ) -> Dict[str, Any]:
        """
    ...
    async def _enhance_plan_with_ai(
        self, task: Task, initial_subtasks: List[Task]
    ) -> Optional[List[Task]]:
...
Provide a structured list of subtasks with clear dependencies."""
                ...
                if result.success:
                    ...
                    if enable_openai and self.openai_manager:
                        await self._create_openai_agent_wrapper(
                            result.agent_id, task, openai_api_key
                        )
    ...
    async def _create_openai_agent_wrapper(
        self, agent_id: str, task: Task, api_key: Optional[str]
    ):
...
Execute this task efficiently and report progress."""
    ...
    def get_hierarchy_visualization(self) -> str:
        ...
        def add_node(parent_tree, node_id: str, depth: int = 0):
            ...
            role = (
                node.agent.config.role.value
                if hasattr(node.agent, 'config')
                else "unknown"
            )
            ...
            if node_id in self.agent_assignments.values():
                task_id = next(
                    tid for tid, aid in self.agent_assignments.items() if aid == node_id
                )
...
async def get_hierarchy():
    ...
    def build_tree(node_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "role": node.agent.config.role.value
            if hasattr(node.agent, 'config')
            else "unknown",
    ...
    async def run_task():
        ...
        with Progress(
            ...
            if not result["success"]:
                console.print(
                    f"[red]Failed to create task: {result.get('error')}[/red]"
                )
                return

            progress.update(
                task_progress, description="Executing plan...", completed=10
            )
            ...
            while plan_id in orchestrator.active_plans:
                ...
                if (
                    completed
                    + sum(
                        1 for t in plan.tasks.values() if t.status == TaskStatus.FAILED
                    )
                    >= total
                ):
                    break
...
def demo():
    ...
    async def run_demo():
        ...
        for i, task_desc in enumerate(demo_tasks, 1):
            ...
            if result["success"]:
                console.print(
                    f"PASS Created plan with {result['total_subtasks']} subtasks"
                )
                ...
                for level in result["execution_order"][:3]:  # Show first 3 levels
                    console.print(
                        f"  Level {level['level']}: {len(level['parallel_tasks'])} parallel tasks"
                    )
            else:
...
def monitor():
    ...
    async def run_monitor():
        ...
        layout["header"].update(
            Panel(
                "[bold cyan]LlamaAgent Master Program Monitor[/bold cyan]",
                border_style="cyan",
            )
        )
            ...
            while True:
                try:
                    ...
                    status_table.add_row(
                        "Memory Usage", f"{memory_usage.get('percentage', 0):.1f}%"
                    )
                    status_table.add_row(
                        "API Calls", f"{api_usage.get('percentage', 0):.1f}%"
                    )
