...
async def demo_basic_agent() -> None:
    ...
    config = AgentConfig(
        name="DemoAgent", max_iterations=5, spree_enabled=True, debug=False
    )
...
async def demo_spre_planning() -> None:
    ...
    config = AgentConfig(name="SPREAgent", spree_enabled=True, max_iterations=10)
        ...
        if hasattr(app, 'routes'):
            for route in app.routes:
                if hasattr(route, 'methods') and hasattr(route, 'path'):
                    ...
                    methods: Set[str] = cast(
                        Set[str], getattr(route, 'methods', empty_methods)
                    )
                    if methods:
...
def save_demo_results(test_results: List[Tuple[str, bool]]) -> None:
    ...
    results: Dict[str, Any] = {
        ...
        "summary": {
            ...
            "success_rate": sum(1 for _, result in test_results if result)
            / len(test_results)
            if test_results
            else 0,
        },
...
async def main() -> None:
    ...
    print("\nNext steps:")
    print(
        "  1. Start the API server: python -m uvicorn src.llamaagent.api:create_app --factory --reload"
    )
