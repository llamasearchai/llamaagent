...
sys.path.append('.')
from enhanced_working_demo import (AgentConfig, EnhancedAgent,
                                   EnhancedMockProvider, LLMMessage,
                                   LLMResponse)
...
REQUEST_COUNT = Counter(
    'llamaagent_requests_total', 'Total requests', ['method', 'endpoint', 'status']
)
REQUEST_DURATION = Histogram('llamaagent_request_duration_seconds', 'Request duration')
ACTIVE_CONNECTIONS = Gauge(
    'llamaagent_active_connections', 'Active WebSocket connections'
)
...
AGENT_EXECUTION_TIME = Histogram(
    'llamaagent_agent_execution_seconds', 'Agent execution time'
)
LLM_CALLS = Counter(
    'llamaagent_llm_calls_total', 'Total LLM calls', ['provider', 'model']
)
LLM_TOKENS = Counter(
    'llamaagent_llm_tokens_total', 'Total LLM tokens', ['provider', 'type']
)
    ...
    try:
        ...
        if settings.enable_metrics:
            AGENT_EXECUTIONS.labels(
                agent_type=request.agent_type, status="success"
            ).inc()
...
async def chat_completions(request: ChatRequest, user: dict = Depends(verify_token)):
    """OpenAI-compatible chat completions endpoint."""
    start_time = time.time()
    ...
    try:
        ...
        if settings.enable_metrics:
            ...
            LLM_TOKENS.labels(provider="mock", type="completion").inc(
...
async def stream_chat_completions(
    request: ChatRequest, user: dict = Depends(verify_token)
):
        ...
        while True:
            ...
            try:
                ...
                if request.get("type") == "agent_execute":
                    ...
                    # Chat message
                    messages = [
                        LLMMessage(role="user", content=request.get("message", ""))
                    ]
                    response = await enhanced_provider.complete(messages)
...
async def log_agent_execution(
    task_id: str, user: str, task: str, result: str, execution_time: float
):
    """Log agent execution (background task)."""
    logger.info(
        f"Agent execution completed - Task ID: {task_id}, User: {user}, Time: {execution_time:.3f}s"
    )
...
async def log_benchmark_execution(user: str, results: Any, execution_time: float):
    ...
    logger.info(
        f"Benchmark completed - User: {user}, Success Rate: {results.summary.success_rate:.1%}, Time: {execution_time:.3f}s"
    )
