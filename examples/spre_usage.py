...
async def demonstrate_spre_agent() -> AgentResponse:
    """Demonstrate SPRE agent on a complex multi-step task."""

    print_section("SPRE Agent Demonstration", "Strategic Planning & Resourceful Execution in Action")
    ...
    config = AgentConfig(name="SPRE-Demo-Agent", role=AgentRole.PLANNER, spree_enabled=True, max_iterations=10)
    ...
    planning_events = [e for e in response.trace if e.get("type") in ["planning_start", "plan_generated"]]
    resource_events = [e for e in response.trace if e.get("type") == "resource_assessment"]
    tool_events = [e for e in response.trace if e.get("type") in ["tool_execution_start", "tool_execution_success"]]
    synthesis_events = [e for e in response.trace if e.get("type") == "synthesis_complete"]
    ...
    # Show plan details
    plan_event = next((e for e in response.trace if e.get("type") == "plan_generated"), None)
    if plan_event and "plan" in plan_event.get("data", {}):
...
async def run_baseline_comparison() -> Dict[str, Dict[str, Any]]:
    """Run comparison across all baseline types."""

    print_section("Baseline Comparison", "Comparing SPRE against other agent configurations")
    ...
    for baseline_type in BaselineAgentFactory.get_all_baseline_types():
        ...
        api_calls = len(
            [
                ...
                if e.get("type")
                in ["planner_response", "resource_assessment_detail", "internal_execution", "synthesis_complete"]
            ]
        )
    ...
    for baseline_type, result in results.items():
        print(
            f"{baseline_type:<15} {str(result['success']):<8} {result['execution_time']:<10.2f} {result['api_calls']:<10}"
        )

    return results


async def run_full_benchmark() -> Dict[str, BenchmarkResult]:
    """Run full benchmark evaluation."""

    print_section("Full Benchmark Evaluation", "Running comprehensive GAIA benchmark evaluation")

    evaluator = SPREEvaluator()

    # Run evaluation with limited tasks for demo
    results = await evaluator.run_full_evaluation(task_filter={"min_steps": 2}, max_tasks_per_baseline=5)

    print_subsection("Benchmark Results Summary")

    print(f"{'AGENT':<20} {'SUCCESS':<12} {'API CALLS':<15} {'EFFICIENCY':<10}")
    print("-" * 70)

    for _, result in results.items():
        print(
            f"{result.agent_name:<20} {result.success_rate:<12.1f}% {result.avg_api_calls:<15.1f} {result.efficiency_ratio:<10.2f}"
        )
...
async def demonstrate_interactive_planning() -> None:
    """Show interactive planning process."""

    print_section("Interactive Planning Demonstration", "Step-by-step breakdown of SPRE planning process")

    # Create agent with detailed tracing
    config = AgentConfig(name="Interactive-SPRE", role=AgentRole.PLANNER, spree_enabled=True)
    ...
    print_subsection("Full Execution")
    # Since we can't access protected methods, we'll execute the full task
    ...
    if planning_events:
        print_subsection("Planning Details")
        for event in planning_events:
            plan_data = event.get("data", {}).get("plan", {})
            if plan_data:
                print(f"Generated {len(plan_data.get('steps', []))} steps")
                
    if resource_events:
        print_subsection("Resource Assessment Details")
        for event in resource_events:
            data = event.get("data", {})
            step_id = data.get("step_id", "Unknown")
            needs_tool = data.get("needs_tool", False)
...
async def main() -> None:
    ...
    print_section(
        "LlamaAgent SPRE Demonstration Suite", "Comprehensive showcase of Strategic Planning & Resourceful Execution"
    )
    ...
    try:
        ...
        print_section("Demonstration Complete", "SPRE methodology successfully demonstrated across multiple scenarios")
