...
from llamaagent.tools import WebSearchTool, CalculatorTool, CodeInterpreterTool
...
print(f"Sequential:   {sequential_system.name} - {sequential_system.agents[0].name} -> {sequential_system.agents[1].name} -> {sequential_system.agents[2].name}")
print(f"Round-Robin:  {round_robin_system.name} - agents take turns refining the solution")
print(f"Hierarchical: {hierarchical_system.name} - {hierarchical_system.agents[0].name} delegates to and synthesizes from others")
print("=" * 70) 