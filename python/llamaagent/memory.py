...
class Memory:
    ...
    def add(self, item: Union[Dict[str, Any], MemoryItem]) -> str:
        ...
        if not isinstance(item, MemoryItemImpl):
            ...
            if hasattr(item, "id") and hasattr(item, "content") and hasattr(item, "metadata") and hasattr(item, "timestamp"):
        ...
        if self.config.short_term_limit and len(self._items) > self.config.short_term_limit:
        ...
        if self.vector_store and self.embedding_function:
            try:
                ...
                content_str = str(memory_item.content) if not isinstance(memory_item.content, str) else memory_item.content
    ...
    def retrieve(
        ...
        if self.vector_store and self.embedding_function:
            try:
                ...
                if results:
                    relevance_score = sum(r.get("score", 0) for r in results) / len(results)
        ...
        if not items or not self.vector_store:
            ...
            # Apply filter if provided
            if filter_func:
                matched_items = [(item, score) for item, score in matched_items if filter_func(item)]
            
            # Extract items and calculate average relevance
            items = [item for item, _ in matched_items]
            if matched_items:
                relevance_score = sum(score for _, score in matched_items) / len(matched_items)
