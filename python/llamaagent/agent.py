...
from .types import AgentState, ToolResult, AgentResponse, Tool
...
class Agent:
    ...
    def run(self, task: str, **kwargs) -> AgentResponse:
        ...
        if self.memory:
            self.memory.add({"type": "task", "content": task, "timestamp": self.state.timestamp})
        ...
        if self.memory and reflection:
            self.memory.add({"type": "reflection", "content": reflection, "timestamp": self.state.timestamp})
    ...
    def _generate_summary(self, state: AgentState) -> str:
        ...
        observations = [result.observation for result in state.results if result.observation]
