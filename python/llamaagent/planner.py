...
from .types import Plan, PlanStep, AgentState
...
class Planner:
    ...
    def create_plan(self, task: str, state: AgentState, memory: Optional[Memory] = None) -> Plan:
        """
        ...
        tools_descriptions = "\n".join(
            [f"- {tool.name}: {tool.description}" for tool in self.config.available_tools]
        )
    ...
    def revise_plan(self, state: AgentState, memory: Optional[Memory] = None) -> Plan:
        ...
        for i, result in enumerate(state.results):
            step_desc = current_plan.steps[i].description if i < len(current_plan.steps) else "Unknown step"
        ...
        remaining_steps_text = "\n".join(
            [f"{i+state.current_step+2}. {step.description}" for i, step in enumerate(remaining_steps)]
        )
