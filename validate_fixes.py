...
def test_monitoring_imports():
    ...
    try:
        from src.llamaagent.monitoring import (
            CircuitBreaker,
            HealthChecker,
            MetricsCollector,
            get_logger,
...
def test_llm_provider_interfaces():
    ...
    try:
        ...
        from src.llamaagent.llm.providers.cohere_provider import CohereConfig, CohereProvider
        from src.llamaagent.llm.providers.openai_provider import OpenAIProvider
        from src.llamaagent.llm.providers.together_provider import TogetherConfig, TogetherProvider
...
def test_security_modules():
    ...
    try:
        from src.llamaagent.security import (
            InputValidator,
            RateLimiter,
            SecurityManager,
        )
    ...
    try:
        ...
        try:
            ...
            if any(dep in str(e) for dep in ["typer", "rich", "click"]):
                print("WARNING  CLI main module: optional dependencies missing (expected)")
            else:
                raise
        ...
        try:
            ...
            if any(dep in str(e) for dep in ["sqlite_utils", "rich", "click"]):
                print("WARNING  CLI LLM interface: optional dependencies missing (expected)")
            else:
                raise
