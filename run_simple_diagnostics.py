...
def main():
    ...
    # Generate report
    generate_report(project_root, problems, len(python_files), total_lines, files_with_errors)
...
def check_imports(py_file: Path, content: str, lines: List[str], problems: List[Dict], project_root: Path):
...
def check_security_patterns(py_file: Path, content: str, lines: List[str], problems: List[Dict], project_root: Path):
...
def check_code_quality(py_file: Path, content: str, lines: List[str], problems: List[Dict], project_root: Path):
...
def generate_report(project_root: Path, problems: List[Dict], total_files: int, total_lines: int, files_with_errors: int):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        if problems_by_category.get('SECURITY_VULNERABILITY'):
            f.write("SECURITY Address security vulnerabilities to protect against attacks\n")
        if problems_by_category.get('CONFIGURATION_ERROR'):
            f.write("Analyzing  Fix configuration errors for proper system operation\n")
        
        f.write("TARGET Focus on critical and high-priority issues first for maximum impact\n")
        f.write("\n")
        ...
        f.write(f"Critical Issue Rate: {critical_count/total_files:.2f} critical issues per file\n")
