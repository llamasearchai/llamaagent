...
class GDTDataset:
    ...
    def __init__(self, name: str, description: str = "", items: Optional[List[GDTItem]] = None):
        self.name = name
    ...
    def from_dict(cls, data: Dict[str, Any]) -> GDTDataset:
        ...
        dataset.created_at = datetime.fromisoformat(data["created_at"]) if "created_at" in data else datetime.now(timezone.utc)
        dataset.metadata = data.get("metadata", {})
        dataset.items = [GDTItem.from_dict(item_data) for item_data in data.get("items", [])]
        return dataset
...
class GDTValidator:
    ...
    def validate_item(self, item: GDTItem) -> Dict[str, Any]:
        ...
        if item.data_type in [DataType.TEXT, DataType.INSTRUCTION]:
            ...
            if len(text_content) < self.rules["min_content_length"]:
                warnings.append(f"Content length ({len(text_content)}) below minimum ({self.rules['min_content_length']})")
            elif len(text_content) > self.rules["max_content_length"]:
                errors.append(f"Content length ({len(text_content)}) exceeds maximum ({self.rules['max_content_length']})")
    ...
    def validate_data(self, data: Union[GDTItem, GDTDataset, List[GDTItem]]) -> List[Dict[str, Any]]:
...
class GDTTransformer:
    ...
    def transform_item(self, item: GDTItem, transformation: str, **kwargs: Any) -> GDTItem:
    ...
    def transform_data(self, data: Union[GDTItem, GDTDataset], transformation: str, **kwargs: Any) -> Union[GDTItem, GDTDataset]:
...
class ConversationDataGenerator(DataGenerator):
    ...
    def generate_item(self, **kwargs: Any) -> GDTItem:
        ...
        return GDTItem(
            ...
            metadata={"generator": "ConversationDataGenerator", "generation_params": kwargs}
        )
...
class GDTGenerator:
    ...
    def generate_dataset(self, name: str, data_type: DataType, count: int, **kwargs: Any) -> GDTDataset:
        ...
        dataset = GDTDataset(name, f"Generated dataset of {count} {data_type.value} items")
