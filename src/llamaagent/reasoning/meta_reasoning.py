...
class StrategyPerformance:
    ...
    performance_by_complexity: Dict[ProblemComplexity, float] = field(default_factory=dict)
    ...
    def update_performance(self, success: bool, confidence: float, 
        ...
        if complexity not in self.performance_by_complexity:
            ...
            self.performance_by_complexity[complexity] = (
                (1 - alpha) * self.performance_by_complexity[complexity] + 
                alpha * confidence
            )
...
class ProblemAnalysis:
    ...
    def __post_init__(self):
        ...
        self.sentence_count = len([s for s in self.problem_text.split('.') if s.strip()])
...
"""
        ...
        try:
            ...
            if analysis_data:
                analysis = ProblemAnalysis(
                    ...
                    complexity=ProblemComplexity(analysis_data.get("complexity", "moderate")),
                    ...
                    requires_factual_knowledge=analysis_data.get("requires_factual_knowledge", False),
                    requires_mathematical_reasoning=analysis_data.get("requires_mathematical_reasoning", False),
                    requires_ethical_consideration=analysis_data.get("requires_ethical_consideration", False),
    ...
    def _fallback_analysis(self, problem_text: str) -> ProblemAnalysis:
        ...
        analysis.requires_multi_step = any(word in text_lower for word in 
            ["then", "next", "after", "step", "first", "second", "finally"])
        
        analysis.requires_mathematical_reasoning = any(word in text_lower for word in
            ["calculate", "compute", "math", "equation", "formula", "number"])
        
        analysis.requires_ethical_consideration = any(word in text_lower for word in
            ["should", "ought", "ethical", "moral", "right", "wrong", "fair"])
...
class StrategySelector:
    ...
    def __init__(self, llm_provider: Any):
        ...
        self.strategy_rules = {
            ...
            ProblemComplexity.SIMPLE: [ReasoningStrategy.SIMPLE, ReasoningStrategy.CONSTITUTIONAL],
            ProblemComplexity.MODERATE: [ReasoningStrategy.TREE_OF_THOUGHTS, ReasoningStrategy.CONSTITUTIONAL],
            ProblemComplexity.COMPLEX: [ReasoningStrategy.TREE_OF_THOUGHTS, ReasoningStrategy.GRAPH_OF_THOUGHTS],
            ProblemComplexity.HIGHLY_COMPLEX: [ReasoningStrategy.GRAPH_OF_THOUGHTS, ReasoningStrategy.HYBRID]
        }
    ...
    async def select_strategy(self, problem_text: str, 
                            context: Optional[Dict[str, Any]] = None) -> Tuple[ReasoningStrategy, ProblemAnalysis]:
        ...
        candidates = self.strategy_rules.get(analysis.complexity, [ReasoningStrategy.SIMPLE])
        ...
        if analysis.requires_creativity and analysis.complexity.value in ["complex", "highly_complex"]:
        ...
        logger.info(f"Selected strategy '{selected_strategy.value}' for {analysis.complexity.value} problem")
        
        return selected_strategy, analysis
    
    def _select_best_strategy(self, candidates: List[ReasoningStrategy], 
                            analysis: ProblemAnalysis) -> ReasoningStrategy:
    ...
    def update_strategy_performance(self, strategy: ReasoningStrategy, success: bool,
                                  confidence: float, execution_time: float,
                                  tokens_used: int, problem_analysis: ProblemAnalysis):
...
class ConfidenceSystem:
    ...
    def __init__(self, llm_provider: Any):
        ...
        self.calibration_data: List[Tuple[float, bool]] = []  # (predicted_confidence, actual_success)
...
"""
    ...
    async def assess_confidence(self, problem: str, response: str, 
                              strategy_used: ReasoningStrategy = ReasoningStrategy.SIMPLE) -> Dict[str, Any]:
    ...
    def _parse_confidence_assessment(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if start_idx != -1 and end_idx != 0:
                ...
                assessment["confidence"] = max(0.0, min(1.0, 
                    float(assessment.get("confidence", 0.5))))
    ...
    def _apply_calibration(self, raw_confidence: float, 
        ...
        try:
            ...
            similar_predictions = [
                (conf, success) for conf, success in self.calibration_data
                if abs(conf - raw_confidence) < 0.2
            ]
            
            if len(similar_predictions) >= 5:
                actual_success_rate = sum(success for _, success in similar_predictions) / len(similar_predictions)
    ...
    def get_calibration_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_confidence": statistics.mean(conf for conf, _ in self.calibration_data),
            "success_rate": statistics.mean(success for _, success in self.calibration_data)
        }
...
class MetaCognitiveAgent:
    ...
    async def select_and_execute_strategy(self, problem: str, available_strategies: Dict[str, Any],
                                        context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Select optimal strategy and coordinate execution"""
        start_time = time.time()
        
        # Phase 1: Strategy Selection
        selected_strategy, problem_analysis = await self.strategy_selector.select_strategy(
            problem, context
        )
        ...
        if selected_strategy.value in available_strategies:
            ...
            try:
                ...
                success = True
                response_content = execution_result.get("solution", execution_result.get("answer", ""))
            except Exception as e:
        ...
        self.strategy_selector.update_strategy_performance(
            selected_strategy, success, final_confidence, execution_time,
            tokens_used, problem_analysis
        )
        ...
        self._record_performance(
            problem_analysis, selected_strategy, success, 
            final_confidence, execution_time, execution_result
        )
    ...
    def _update_meta_statistics(self, success: bool, confidence: float):
        ...
        self.stats["average_confidence"] = (
            (1 - alpha) * self.stats["average_confidence"] + alpha * confidence
        )
        
        # Update success rate
        if self.stats["total_decisions"] > 0:
            success_rate = self.stats["successful_decisions"] / self.stats["total_decisions"]
            # Calibration accuracy is how close average confidence is to success rate
            self.stats["calibration_accuracy"] = 1.0 - abs(self.stats["average_confidence"] - success_rate)
    
    def _record_performance(self, problem_analysis: ProblemAnalysis, strategy: ReasoningStrategy,
    ...
    def analyze_performance_patterns(self) -> Dict[str, Any]:
        ...
        trend_analysis: Dict[str, Dict[str, Any]] = {
            strategy: {
                ...
                "confidence_trend": "improving" if len(confidences) > 5 and 
                                 statistics.mean(confidences[-5:]) > statistics.mean(confidences[:5])
                                 else "stable"
            }
    ...
    def _calculate_improvement_trend(self) -> Dict[str, float]:
        ...
        return {
            ...
            "overall_trend": (recent_success + recent_confidence) - (early_success + early_confidence)
        }