...
class Constitution:
    ...
    def get_rules(self, category: Optional[str] = None, enabled_only: bool = True) -> List[ConstitutionalRule]:
...
class CritiqueSystem:
    ...
    def __init__(self, llm_provider: Any):
        ...
        if not hasattr(llm_provider, 'complete') or not callable(getattr(llm_provider, 'complete')):
...
"""
    ...
    async def critique_response(self, response: str, constitution: Constitution) -> Dict[str, Any]:
...
"""
    ...
    async def revise_response(self, original_response: str, violations: List[SafetyViolation], 
    ...
    def _create_fallback_response(self, original_response: str, violations: List[SafetyViolation]) -> str:
...
class ConstitutionalAgent:
    ...
    def __init__(
        ...
        if max_revision_attempts <= 0:
            raise ValueError(f"max_revision_attempts must be positive integer, got {max_revision_attempts}")
...
"""
        ...
        logger.info(f"ConstitutionalAgent initialized with {len(self.constitution.rules)} rules")
    
    async def process_response(self, query: str, enable_revision: Optional[bool] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            critique_result = await self.critique_system.critique_response(initial_response, self.constitution)
            ...
            if enable_revision and violations and revision_attempts < self.max_revision_attempts:
                logger.info(f"Revising response to address {len(violations)} violations...")
                ...
                while violations and revision_attempts < self.max_revision_attempts:
                    ...
                    critique_result = await self.critique_system.critique_response(revised_response, self.constitution)
                    violations = critique_result.get("violations", [])
                    compliance_score = critique_result.get("compliance_score", 1.0)
                    overall_compliance = critique_result.get("overall_compliance", True)
                    ...
                    logger.info(f"Revision attempt {revision_attempts}: {len(violations)} violations remaining")
            ...
            self.stats["compliance_rate"] = compliant_executions / total_executions if total_executions > 0 else 1.0
            ...
            return {
                ...
                "statistics": {
                    ...
                    "rules_checked": len(self.constitution.get_rules(enabled_only=True))
                },
                "success": True,
                "error": None
            }
