...
class SearchStrategyBase(ABC):
    ...
    @abstractmethod
    async def search(self, tree: ThoughtTree, evaluator: ThoughtEvaluator, 
                    problem: str, max_iterations: int) -> List[ThoughtNode]:
...
class MonteCarloTreeSearch(SearchStrategyBase):
    ...
    async def _simulate_and_backpropagate(self, node: ThoughtNode, 
                                        evaluator: ThoughtEvaluator, problem: str) -> None:
...
"""
    ...
    async def solve(self, problem: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Execute search
            best_path = await search_strategy.search(tree, self.evaluator, problem, self.max_iterations)
            ...
            tree_depth = max(node.depth for node in tree.nodes.values()) if tree.nodes else 0
            ...
            self.stats["average_tree_depth"] = (
                (self.stats["average_tree_depth"] * (self.stats["total_executions"] - 1) + tree_depth) 
                / self.stats["total_executions"]
            )
            self.stats["average_nodes_created"] = (
                (self.stats["average_nodes_created"] * (self.stats["total_executions"] - 1) + nodes_created) 
                / self.stats["total_executions"]
            )
