...
"""
    ...
    async def build_and_test(self):
        ...
        console.print("[bold green]PASS Build and test completed successfully![/bold green]")
    ...
    async def show_system_status(self):
        ...
        if api_info:
            table.add_row("Agents", f"PASS {api_info.get('agents_count', 0)}", "Active agents")
            table.add_row("Uptime", f"PASS {api_info.get('uptime', 0):.1f}s", "Server uptime")
            ...
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    ...
                    json={
                        "model": "mock-model",
                        "messages": [
                            {"role": "user", "content": "Hello, can you calculate 15 + 27?"}
                        ]
                    }
                )
                ...
                if response.status_code == 200:
                    ...
                    console.print(f"[red]FAIL Chat completion failed: {response.status_code}[/red]")
                ...
                if response.status_code == 200:
                    ...
                    console.print(f"[red]FAIL Agent creation failed: {response.status_code}[/red]")
    ...
    async def show_configuration(self):
        ...
        env_vars = [
            ...
            "LLAMAAGENT_LLM_PROVIDER", "LLAMAAGENT_LLM_MODEL"
        ]
